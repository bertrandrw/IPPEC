...
// --- PATIENT CONTROLLERS ---
export const listMyOrdersController = catchAsync(async (req, res) => {
    const patientUser = req.user;
...
export const getMyOrderByIdController = catchAsync(async (req, res) => {
    const patientUser = req.user;
    const { id } = req.params;
    // --- THIS IS THE FIX ---
    if (!id) {
...
export const listPharmacyOrdersController = catchAsync(async (req, res) => {
    const pharmacistUser = req.user;
    const orders = await orderService.listPharmacyOrders(pharmacistUser);
    res.status(StatusCodes.OK).json(new ApiResponse(StatusCodes.OK, orders));
});
export const getPharmacyOrderByIdController = catchAsync(async (req, res) => {
    const pharmacistUser = req.user;
    const { id } = req.params;
    // --- THIS IS THE FIX ---
    if (!id) {
