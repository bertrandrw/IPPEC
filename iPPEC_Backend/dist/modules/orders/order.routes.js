...
router.post('/', authMiddleware, roleMiddleware([ROLES.PATIENT]), validate(createOrderDto), controller.createOrderController);
// --- PATIENT: LIST OWN ORDERS (Static path, defined early) ---
router.get('/', // This now becomes the patient's "list my orders" route
authMiddleware, roleMiddleware([ROLES.PATIENT]), controller.listMyOrdersController);
// --- PHARMACIST: LIST PHARMACY ORDERS (Specific static path) ---
router.get('/pharmacy', // This specific path will now be matched correctly
authMiddleware, roleMiddleware([ROLES.PHARMACIST]), controller.listPharmacyOrdersController);
// --- PHARMACIST: GET/UPDATE A SPECIFIC ORDER (Specific dynamic path) ---
router.get('/pharmacy/:id', authMiddleware, roleMiddleware([ROLES.PHARMACIST]), controller.getPharmacyOrderByIdController);
router.patch('/pharmacy/:id/status', authMiddleware, roleMiddleware([ROLES.PHARMACIST]), validate(updateOrderStatusDto), controller.updateOrderStatusController);
// --- PATIENT: GET A SPECIFIC ORDER (Most generic dynamic route, defined LAST) ---
router.get('/:id', authMiddleware, roleMiddleware([ROLES.PATIENT]), controller.getMyOrderByIdController);
