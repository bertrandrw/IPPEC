...
router.get('/patient/me', // This is highly specific
authMiddleware, roleMiddleware([ROLES.PATIENT]), controller.listMyPrescriptionsController);
router.get('/:prescriptionId/find-pharmacies', authMiddleware, roleMiddleware([ROLES.PATIENT]), // Only patients can find pharmacies for their own prescriptions
validate(findPharmaciesDto), controller.findPharmaciesForPrescriptionController);
// DOCTOR: List own prescriptions
router.get('/my-prescriptions', // This is also specific
authMiddleware, roleMiddleware([ROLES.DOCTOR]), controller.listMyPrescriptionsForDoctorController);
// --- More Generic Routes ---
// DOCTOR: Create a prescription
router.post('/', authMiddleware, roleMiddleware([ROLES.DOCTOR]), validate(createPrescriptionDto), controller.createPrescriptionController);
// --- Routes with Dynamic Parameters ---
// DOCTOR: List for a specific patient
router.get('/patient/:patientId', // This is dynamic, but more specific than /:id
authMiddleware, roleMiddleware([ROLES.DOCTOR]), controller.listPatientPrescriptionsForDoctorController);
// PHARMACIST: Get a prescription for fulfillment
router.get('/pharmacist/fulfill/:id', // Specific prefix makes it non-conflicting
authMiddleware, roleMiddleware([ROLES.PHARMACIST]), controller.getPrescriptionForFulfillmentController);
// PHARMACIST: Fulfill a prescription
router.patch('/pharmacist/fulfill/:id', // Specific prefix
authMiddleware, roleMiddleware([ROLES.PHARMACIST]), controller.fulfillPrescriptionController);
// DOCTOR: Cancel a prescription
router.patch('/:id/cancel', // More specific than /:id
authMiddleware, roleMiddleware([ROLES.DOCTOR]), controller.cancelMyPrescriptionController);
// --- Most Generic Dynamic Routes (LAST) ---
// PATIENT & DOCTOR: View a single prescription
router.get('/:id', authMiddleware, roleMiddleware([ROLES.PATIENT, ROLES.DOCTOR]), controller.getPrescriptionByIdController);
// DOCTOR: Update a single prescription
router.patch('/:id', authMiddleware, roleMiddleware([ROLES.DOCTOR]), validate(updatePrescriptionDto), controller.updateMyPrescriptionController);
