// src/modules/insurers/insurer.controller.ts
...
export const listCompanyClaimsController = catchAsync(async (req: IAuthRequest, res: Response) => {
  ...
  const { page, limit, pharmacyId } = req.query as unknown as {
    page: number;
    limit: number;
    pharmacyId?: string;
  };
...
export const getClaimDetailsController = catchAsync(async (req: IAuthRequest, res: Response) => {
  const insurerUser = req.user!;
  const { id } = req.params;
  if (!id) throw new ApiError(StatusCodes.BAD_REQUEST, 'Claim ID is required.');
...
export const updateClaimStatusController = catchAsync(async (req: IAuthRequest, res: Response) => {
  const insurerUser = req.user!;
  const { id } = req.params;
  if (!id) throw new ApiError(StatusCodes.BAD_REQUEST, 'Claim ID is required.');
...
export const listNetworkPharmaciesController = catchAsync(async (req: IAuthRequest, res: Response) => {
  const insurerUser = req.user!;
...
export const addPharmacyToNetworkController = catchAsync(async (req: IAuthRequest, res: Response) => {
  const insurerUser = req.user!;
...
export const removePharmacyFromNetworkController = catchAsync(async (req: IAuthRequest, res: Response) => {
  const insurerUser = req.user!;
  const { pharmacyId } = req.params;
  if (!pharmacyId) throw new ApiError(StatusCodes.BAD_REQUEST, 'Pharmacy ID is required.');
...
export const adjudicateClaimItemController = catchAsync(async (req: IAuthRequest, res: Response) => {
  const insurerUser = req.user!;
...
export const addPatientToCoverageController = catchAsync(async (req: IAuthRequest, res: Response) => {
  const insurerUser = req.user!;
...
export const removePatientFromCoverageController = catchAsync(async (req: IAuthRequest, res: Response) => {
  const insurerUser = req.user!;
