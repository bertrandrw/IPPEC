...
type AddPatientCoverageInput = z.infer<typeof addPatientCoverageDto>['body'];

type CreateAgreementInput = z.infer<typeof createAgreementDto>['body'];

type UpdateClaimStatusInput = z.infer<typeof updateClaimStatusDto>['body'];

type AdjudicateItemInput = z.infer<typeof adjudicateItemDto>['body'];

type AgreementWithPharmacy = Prisma.InsuranceAcceptanceGetPayload<{
 ...
 */
const getInsurerCompanyId = async (insurerUser: User): Promise<string> => {
...
export const listCompanyClaims = async (insurerUser: User, { page, limit, pharmacyId }: ListClaimsParams) => {
 ...
 */
export const updateClaimStatus = async (insurerUser: User, claimId: string, input: UpdateClaimStatusInput) => {
  const companyId = await getInsurerCompanyId(insurerUser);
...
export const listNetworkPharmacies = async (insurerUser: User, { page, limit }: ListPharmaciesParams) => {
  const companyId = await getInsurerCompanyId(insurerUser);
  const skip = (page - 1) * limit;
  ...
  const data = agreements.map((agreement: AgreementWithPharmacy) => ({
    ...agreement.pharmacy
  }));
 ...
 */
export const addPharmacyToNetwork = async (insurerUser: User, input: CreateAgreementInput) => {
  const companyId = await getInsurerCompanyId(insurerUser);
 ...
 */
export const removePharmacyFromNetwork = async (insurerUser: User, pharmacyId: string) => {
  const companyId = await getInsurerCompanyId(insurerUser);
...
export const adjudicateClaimItem = async (insurerUser: User, claimItemId: string, input: AdjudicateItemInput) => {
  const companyId = await getInsurerCompanyId(insurerUser);
...
export const addPatientToCoverage = async (insurerUser: User, input: AddPatientCoverageInput) => {
  const companyId = await getInsurerCompanyId(insurerUser);
...
export const removePatientFromCoverage = async (insurerUser: User, patientProfileId: string) => {
  const companyId = await getInsurerCompanyId(insurerUser);
