...
type CreateOrderInput = z.infer<typeof createOrderDto>['body'];


// --- PATIENT-SPECIFIC SERVICES ---

export const createOrder = async (patientUser: User, input: CreateOrderInput) => {
 ...
 */
export const listMyOrders = async (patientUser: User) => {
    const patientProfile = await prisma.patientProfile.findUnique({ where: { userId: patientUser.id } });
    if (!patientProfile) throw new ApiError(StatusCodes.NOT_FOUND, 'Patient profile not found.');
   ...
   */
  export const getMyOrderById = async (patientUser: User, orderId: string) => {
    const patientProfile = await prisma.patientProfile.findUnique({ where: { userId: patientUser.id } });
    if (!patientProfile) throw new ApiError(StatusCodes.NOT_FOUND, 'Patient profile not found.');
    ...
    if (!order) throw new ApiError(StatusCodes.NOT_FOUND, 'Order not found or you are not authorized to view it.');
    return order;
  };
   ...
   */
  export const listPharmacyOrders = async (pharmacistUser: User) => {
   ...
   */
  export const updateOrderStatus = async (pharmacistUser: User, orderId: string, newStatus: z.infer<typeof updateOrderStatusDto>['body']['status']) => {
    const pharmacistProfile = await prisma.pharmacistProfile.findUnique({ where: { userId: pharmacistUser.id } });
    if (!pharmacistProfile) throw new ApiError(StatusCodes.FORBIDDEN, 'Pharmacist profile not found.');
