...
router.get(
  '/search',
  authMiddleware,
  validate(searchPharmaciesDto),
  controller.searchPharmaciesController
);

// --- PHARMACIST-ONLY ROUTES ---
const pharmacistRouter = Router();
pharmacistRouter.use(authMiddleware, roleMiddleware([ROLES.PHARMACIST, ROLES.ADMIN, ROLES.INSURER]));
...
adminRouter.use(authMiddleware, roleMiddleware([ROLES.ADMIN, ROLES.INSURER]));
