...
type CreatePrescriptionInput = z.infer<typeof createPrescriptionDto>['body'];
type UpdatePrescriptionInput = z.infer<typeof updatePrescriptionDto>['body'];

interface FindPharmaciesParams {
  latitude: number;
  longitude: number;
  radius: number;
  limit: number;
...
const getDoctorProfile = async (doctorUser: User) => {
...
export const createPrescription = async (doctorUser: User, input: CreatePrescriptionInput) => {
...
export const getPrescriptionByIdForPatient = async (patientUser: User, prescriptionId: string) => {
  const patientProfile = await prisma.patientProfile.findUnique({
    where: { userId: patientUser.id },
  });
 ...
 */
export const getPrescriptionForFulfillment = async (prescriptionId: string) => {
  const prescription = await prisma.prescription.findUnique({
    where: {
      id: prescriptionId,
 ...
 */
export const fulfillPrescription = async (pharmacistUser: User, prescriptionId: string) => {
 ...
 */
export const listMyPrescriptionsForDoctor = async (doctorUser: User, page: number, limit: number) => {
 ...
 */
export const listPrescriptionsForPatientByDoctor = async (patientId: string) => {
 ...
 */
export const getPrescriptionById = async (user: User, prescriptionId: string) => {
  const prescription = await prisma.prescription.findUnique({
    where: { id: prescriptionId },
    include: {
...
export const cancelMyPrescription = async (doctorUser: User, prescriptionId: string) => {
  const doctorProfile = await getDoctorProfile(doctorUser);
...
export const findPharmaciesForPrescription = async (patientUser: User, prescriptionId: string, params: FindPharmaciesParams) => {
  ...
  if (!patientProfile) throw new ApiError(StatusCodes.FORBIDDEN, "Patient profile not found.");
  ...
  if (!prescription) throw new ApiError(StatusCodes.NOT_FOUND, 'Prescription not found or you are not authorized.');
  if (prescription.medicines.length === 0) throw new ApiError(StatusCodes.BAD_REQUEST, 'This prescription has no medicines.');
