...
interface EditableField {
  field: string;
  label: string;
  type: 'text' | 'email' | 'tel' | 'textarea' | 'password' | 'date' | 'select';
  value: string;
  required?: boolean;
  readonly?: boolean;
  options?: string[];
...
const DoctorProfile: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'profile' | 'professional' | 'security' | 'activity'>('profile');

  // Mock additional profile data that would come from database
  const profileData = {
    ...user,
    fullName: user?.name || 'Doctor',
    licenseNumber: user?.licenseNumber || 'MD-RW-2024-001',
    ...
    lastLogin: '2024-10-02 08:15 AM',
    address: 'KG 789 St, Nyarugenge, Kigali',
    bio: 'Experienced internal medicine physician dedicated to providing comprehensive patient care and medical education. Specialized in chronic disease management and preventive medicine.',
    education: [
      {
    ...
    certifications: [
      'Rwanda Medical Council License',
    ...
    stats: {
      ...
      yearsOfPractice: 12,
      specialtyRanking: 3, // Top 3 in specialty
      researchPapers: 18,
      mentorshipPrograms: 5
    }
  };
  ...
  const editableFields: EditableField[] = [
    ...
    { field: 'consultationFee', label: 'Consultation Fee (RWF)', type: 'text', value: profileData.consultationFee.toString(), required: true },
    { field: 'licenseNumber', label: 'Medical License Number', type: 'text', value: profileData.licenseNumber, readonly: true },
  ];

  const professionalFields: EditableField[] = [
    { 
    ...
    { field: 'position', label: 'Position', type: 'text', value: profileData.position, required: true },
    { field: 'department', label: 'Department', type: 'text', value: profileData.department, required: true },
    { field: 'hospitalName', label: 'Hospital/Clinic', type: 'text', value: profileData.hospitalName, required: true },
  ];
  ...
  const handleSave = async (field: string) => {
    try {
      // Here you would make an API call to update the doctor profile
        ...
        <div className="bg-gradient-to-r from-[#397dc0] to-blue-700 rounded-2xl p-8 text-white">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-6">
              ...
              <div>
                <h1 className="text-3xl font-bold mb-2">Dr. {profileData.fullName}</h1>
                <p className="text-blue-100 text-lg mb-2">{profileData.specialization}</p>
                <div className="flex items-center space-x-4">
                  <div className="flex items-center space-x-2">
                    <Building2 className="w-4 h-4" />
                    <span className="text-sm">{profileData.hospitalName}</span>
                  </div>
                  <div className="flex items-center space-x-2">
            ...
            <div className="text-right">
              <div className="flex items-center space-x-2 mb-2">
                ...
                <span className="text-xl font-bold">{profileData.stats.avgPatientRating}/5.0</span>
              </div>
              <p className="text-blue-100 text-sm">Patient Rating</p>
              <div className="mt-2">
                <div className="text-lg font-semibold">{formatCurrency(profileData.consultationFee)}</div>
                <p className="text-blue-100 text-xs">Consultation Fee</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="p-6">
            ...
            {activeTab === 'profile' && (
              <div className="space-y-8">
                        ...
                        {isEditing === field.field ? (
                          ...
                          <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                            <span className="text-gray-900">
                              {field.field === 'consultationFee' ? formatCurrency(parseInt(field.value)) : field.value}
                            </span>
                            {!field.readonly && (
                              <button
                ...
                {/* Contact Information */}
                <div>
                  <h3 className="text-lg font-semibold text-gray-900 mb-6">Contact & Location</h3>
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div className="bg-blue-50 rounded-lg p-4">
                      <div className="flex items-center space-x-3 mb-3">
                        <MapPin className="w-5 h-5 text-[#397dc0]" />
                        <h4 className="font-medium text-gray-900">Practice Location</h4>
                      </div>
                      <p className="text-sm text-gray-600 mb-2">{profileData.hospitalName}</p>
                    ...
                    <div className="bg-green-50 rounded-lg p-4">
                      ...
                      <p className="text-sm text-gray-600 mb-2">Phone: {profileData.phone}</p>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    <div className="bg-blue-50 rounded-lg p-4 text-center">
                      <div className="text-2xl font-bold text-[#397dc0] mb-1">{profileData.stats.totalPatients.toLocaleString()}</div>
                      <div className="text-sm text-gray-600">Total Patients</div>
                    </div>
                    <div className="bg-green-50 rounded-lg p-4 text-center">
                      <div className="text-2xl font-bold text-green-600 mb-1">{profileData.stats.consultationsThisMonth}</div>
                      <div className="text-sm text-gray-600">This Month</div>
                    </div>
                    <div className="bg-orange-50 rounded-lg p-4 text-center">
                      <div className="text-2xl font-bold text-orange-600 mb-1">{profileData.stats.activePrescriptions}</div>
                      <div className="text-sm text-gray-600">Active Prescriptions</div>
                    </div>
                    <div className="bg-purple-50 rounded-lg p-4 text-center">
                      <div className="text-2xl font-bold text-purple-600 mb-1">{profileData.stats.researchPapers}</div>
                      ...
                      <div key={index} className="bg-gray-50 rounded-lg p-4">
                        <div className="flex items-start justify-between">
                          <div className="flex items-start space-x-3">
                            <GraduationCap className="w-5 h-5 text-[#397dc0] mt-1" />
                            <div>
              ...
              <div className="space-y-8">
                <div>
                  ...
                  <div className="bg-gray-50 rounded-lg p-6">
                    <h4 className="font-medium text-gray-900 mb-4">Professional Verification</h4>
                    <div className="space-y-3">
                      <div className="flex items-center justify-between py-2">
                        <div className="flex items-center space-x-3">
                          <CheckCircle className="w-5 h-5 text-green-500" />
                          <span className="text-sm text-gray-900">Medical License Verified</span>
                        </div>
                      ...
                      <div className="flex items-center justify-between py-2">
                        <div className="flex items-center space-x-3">
                          <CheckCircle className="w-5 h-5 text-green-500" />
                          <span className="text-sm text-gray-900">Board Certification</span>
                        </div>
                      ...
                      <div className="flex items-center justify-between py-2">
                        <div className="flex items-center space-x-3">
                          <CheckCircle className="w-5 h-5 text-green-500" />
                          <span className="text-sm text-gray-900">Hospital Affiliation</span>
                        </div>
            ...
            {activeTab === 'activity' && (
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  {[
                    { action: 'Prescribed medication for patient #P2024-1567', time: '1 hour ago', type: 'prescription', icon: FileText },
                    { action: 'Completed consultation with new patient', time: '2 hours ago', type: 'consultation', icon: Stethoscope },
                    { action: 'Updated patient medical record', time: '4 hours ago', type: 'record', icon: User },
                    { action: 'Reviewed and approved prescription request', time: '1 day ago', type: 'approval', icon: CheckCircle },
                    { action: 'Updated professional profile information', time: '2 days ago', type: 'profile', icon: Edit3 },
                  ].map((activity, index) => (
                    <div key={index} className="flex items-start space-x-4 p-4 bg-gray-50 rounded-lg">
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                        activity.type === 'prescription' ? 'bg-blue-100' :
                        activity.type === 'consultation' ? 'bg-green-100' :
                        activity.type === 'record' ? 'bg-orange-100' :
                        activity.type === 'registration' ? 'bg-purple-100' :
                        activity.type === 'approval' ? 'bg-teal-100' :
                        'bg-gray-100'
                      }`}>
                        <activity.icon className={`w-5 h-5 ${
                          activity.type === 'prescription' ? 'text-blue-600' :
                          activity.type === 'consultation' ? 'text-green-600' :
                          activity.type === 'record' ? 'text-orange-600' :
                          activity.type === 'registration' ? 'text-purple-600' :
                          activity.type === 'approval' ? 'text-teal-600' :
                          'text-gray-600'
                        }`} />
                      </div>
